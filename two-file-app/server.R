...
server <- function(input, output) {
  ...
  trout_filtered_df <- reactive({
    ...
    clean_trout |> 
      ...
      filter(section %in% c(input$selection_input))
  })
  
  # Render trout scatter plot
  output$channel_type_output <- renderPlot({
  ...
  island_df <- reactive({
    ...
    validate(need(length(input$island_input) > 0, "Please select at least one island to visualize data for."))
  ...
  # Render island histogram ----
  output$penguins_output <- renderPlot({
